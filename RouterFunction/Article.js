...
exports.article_list = async (req, res) => {
  ...
  if (page === 'all') {
    ...
    const GetAllButDeletedArticlesSql = `SELECT * FROM ev_articles where is_delete=0`
    const GetAllButDeletedArticles = await ExecuteFunc(
      GetAllButDeletedArticlesSql
    )
    ...
    res.status(200).send({
      status: 200,
      message: '获取成功',
      data: GetAllButDeletedArticles,
    })
  } else {
    // 每次只获取10条文章 Get only 10 articles at a time
    const GetOnly10ArticlesAtATimeSql = `SELECT * FROM ev_articles where is_delete=0 ORDER BY ev_articles.id DESC limit 10 offset ?`
    db.query(GetOnly10ArticlesAtATimeSql, parseInt(page), (err, results) => {
...
exports.article_upimage = async (req, res) => {
  ...
  if (QueryTheNumberOfPicturesUploadedToday.length <= setting.MaxFile) {
