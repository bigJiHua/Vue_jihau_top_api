...
exports.SelectData = async (req, res) => {
  ...
  if (GetType === 'article') {
    ...
    stateCondition = ' WHERE is_delete = 0'
...
exports.cagUAData = async (req, res) => {
  ...
  if (checkIsChange.length !== 1) {
    ...
    // 增加修改次数
    const UpdateStubDataSql = `UPDATE ev_cagarticlelog  SET ? WHERE article_id =? AND username =? AND change_num<=30`
    const UpdateStubData = await ExecuteFuncData(UpdateStubDataSql, [
      BackupData,
      tablevalue,
      username,
    ])
...
async function UpdateNumAndData(decryptedData, username, tableName, tablevalue, tableId, res) {
  ...
  const UpdateUserArticle = await ExecuteFuncData(UpdateUserArticleSql, [
    decryptedData,
    tablevalue,
    username,
  ])
...
exports.searchanything = async (req, res) => {
  ...
  switch (GetType) {
    ...
    case 'user':
      tableName = 'ev_users'
      stateCondition = ' AND useridentity = "user"'
      SelectId = 'user_id'
      break
    case 'manager':
      tableName = 'ev_users'
...
exports.postNotify = async (req, res) => {
  ...
  if (!postId) {
    ...
    if (CheckIfThereAreDuplicateTiTles.length !== 0)
      return res.cc('文章已经被发布过啦！请换一个标题吧!', 409)
    // 插入文章 Insert article
