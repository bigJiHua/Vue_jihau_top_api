...
const setUserPXData = async (req, res) => {
  if (!req.headers.viewportwidth) return
  ...
  const UserPXData = {
    ...
    browser_info: '',
    engine_info: '',
    viewport_width: req.headers.viewportwidth,
    viewport_height: req.headers.viewportheight,
    pixel_ratio: req.headers.pixelratio,
    navigator_platform: req.headers.navigatorplatform || '',
    created_at: new Date().getTime(),
  }
  try {
    // 尝试从 user-agent 中提取信息
    ...
    const engineMatch = ua.match(/(AppleWebKit|Gecko|Trident)\/[^\s]+/)
    UserPXData.engine_info = engineMatch ? engineMatch[0] : 'Unknown Engine'
  } catch (err) {
    console.error('setUserPXData UA parsing error:', err)
  }

  try {
    // 查询是否存在相同宽高
    const CheckisWHSql = `SELECT * FROM ev_usereqm WHERE viewport_width = ? AND viewport_height = ?`
    const CheckisWH = await ExecuteFuncData(CheckisWHSql, [
      UserPXData.viewport_width,
      UserPXData.viewport_height,
    ])

    if (CheckisWH.length > 0) {
      // 已存在则直接累加 count
      const updateSQL = `UPDATE ev_usereqm SET count = count + 1 WHERE viewport_width = ? AND viewport_height = ?`
      await ExecuteFuncData(updateSQL, [UserPXData.viewport_width, UserPXData.viewport_height])
    } else {
      // 不存在则插入新纪录
    ...
    console.error('setUserPXData DB error:', err, UserPXData)
  }
}

module.exports = { setUserPXData }


/*
* 2025年10月4日 下面是改进前的代码
*/
// const ExecuteFuncData = require('../Implement/ExecuteFunctionData')
...
//         `Update ev_usereqm set count = count + 1 where viewport_width = ? and viewport_height = ?`,
//         [UserPXData.viewport_width, UserPXData.viewport_height],
//       )
//       return
