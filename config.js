...
const config = {
  ...
  path: './public/uploads/',
  // 文件访问路径
  selpath: 'http://192.168.0.103:666/api/public/uploads/',
  // Mail发送邮箱
  sendMail: '',
  // Mail发送密码
  sendPassword: '',
  ...
  // session密钥
  sessionKey: 'jihua is a good man !',
  // 校验规则
  get options() {
    return {
  ...
  // 挑选内容
  SelectContent(data, limit) {
    const CZFilterData = []
    data.forEach((value) => {
      const data = value
      data.content =
        this.FilterContent(data.content, limit) === ''
          ? data.title
          : this.FilterContent(data.content, limit)
